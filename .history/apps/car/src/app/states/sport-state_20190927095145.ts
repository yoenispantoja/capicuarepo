...
export class SportState {
  servicio: SportService;


  @Selector() //Lo usamos para utilizar las acciones de selección
  static getSports(stateSport: StateContext<SportStateModel>) {
    //Consumo los deportes desde el servicio
    let servicio: SportService;
    const state = stateSport.getState();
    servicio.getSports().subscribe(data => {
      stateSport.setState({
        ...state,
        sports: [
          ...state.sports,
      ...
      console.log('prueba: ' + state.sports);
      return state.sports;
    })
  }
